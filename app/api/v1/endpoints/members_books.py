...
@router.get("/")
def get_members_books(
    db: DB, skip: int = 0, limit: int = 100
) -> list[schemas.MembersBooks]:
    """
...
def get_members_books_by_id(db: DB, book_id: int) -> list[schemas.MembersBooks]:
    ...
    return [
        schemas.MembersBooks.from_orm(members_books)
        for members_books in crud.members_books.get_members_books_by_book_id(
            db, book_id=book_id
        )
    ]
...
def get_books_by_member_id(db: DB, member_id: int) -> list[schemas.MembersBooks]:
    ...
    return [
        schemas.MembersBooks.from_orm(members_books)
        for members_books in crud.members_books.get_members_books_by_member_id(
...
def get_members_books_by_member_id_and_book_id(
    db: DB, member_id: int, book_id: int
) -> schemas.MembersBooks:
    """
    An endpoint that returns a member-book record from the database.
    """
    return schemas.MembersBooks.from_orm(
        crud.members_books.get_members_books_by_member_id_and_book_id(
...
def update_members_books(
    ...
    if members_books := crud.members_books.get_members_books_by_member_id_and_book_id(
        ...
        return schemas.MembersBooks.from_orm(
            crud.members_books.update(
                db, db_obj=members_books, obj_in=members_books_update
            )
        )
